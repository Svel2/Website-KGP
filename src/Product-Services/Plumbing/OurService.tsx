...
export default function OurService() {
                        ...
                        {projects.map((project, index) => (
                            <DirectionAwareHover key={index} imageUrl={project.imageUrl}>
                                <div className="p-4 sm:p-6">
                                    <h3 className="text-lg sm:text-xl font-bold text-white mb-2 sm:mb-3">{t(`ourService.projects.plumbing.${project.key}.title`)}</h3>
                                    <p className="text-gray-300 text-sm sm:text-base leading-relaxed">{t(`ourService.projects.plumbing.${project.key}.description`)}</p>
                                </div>
                            </DirectionAwareHover>
                        ))}
                    </div>
                                ...
                                <div className="absolute bottom-0 inset-x-0 z-10">
                                    <div className="flex flex-col h-full p-4 sm:p-6">
                                        <h3 className="text-lg sm:text-2xl font-semibold text-white group-hover:text-white/80 group-focus:text-white/80">
                                            {t(`ourService.projects.plumbing.${project.key}.title`)}
                                        </h3>
                                        <p className="mt-2 text-white/80 text-sm sm:text-base">
                                            {t(`ourService.projects.plumbing.${project.key}.description`)}
                                        </p>
                                    </div>
                                </div>
                            </a>
    ...
    const projects = [
        {
            key: "systemInstallation",
            imageUrl: "/images/plumbing-service1.jpg",
        },
        {
            key: "maintenanceRepair",
